...
- クールダウン (5分): 4.5マイル/hから徐々に3.5マイル/hに下げる`
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'} flex flex-col`}>
      {/* Header */}
      <header className={`${darkMode ? 'glass-effect-dark' : 'bg-white shadow-lg'}`}>
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <span className="hidden sm:inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-purple-600 to-pink-600 text-white">
        ...
        <div className="flex-1 max-w-7xl mx-auto px-4 py-6 w-full">
          {activeTab === 'prompt' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Prompt Area */}
              <div className={`${darkMode ? 'glass-effect-dark' : 'bg-white'} rounded-2xl p-6`}>
          ...
          {activeTab === 'profile' && (
            <div className={`max-w-2xl mx-auto ${darkMode ? 'glass-effect-dark' : 'bg-white'} rounded-2xl p-6`}>
                ...
                {context && (
                  <div className={`mt-6 p-4 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                    <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      現在のセッション: {context.sessionNumber} ({getSessionTitle(context.sessionNumber)})
                    </p>
                    <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      サイクル: {context.cycleNumber}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className={`p-4 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                  ...
                  <ol className={`list-decimal list-inside space-y-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    <li>「プロフィール」タブで情報を入力</li>
                    <li>「プロンプト」タブでプロンプトをコピー</li>
                    <li>Claude AIに貼り付けて実行</li>
                    <li>Claudeの結果を「Claudeの結果」エリアに貼り付け</li>
                    <li>自動的に次のセッションのプロンプトが生成されます</li>
                  </ol>
                </div>
                ...
                <div className={`p-4 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    8セッションで1サイクルです。サイクル完了後、トレーニングメニューが自動的に再構成されます。
                  </p>
                </div>
              </div>
            </div>
      ...
      {error && activeTab === 'prompt' && (
        <div className="fixed bottom-20 right-4 bg-red-500 text-white px-4 py-3 rounded-xl shadow-lg fade-in">
          {error}
          <button
